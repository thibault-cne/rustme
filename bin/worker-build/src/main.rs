...
fn create_worker_dir() -> Result<()> {
    ...
    let worker_dir = base_worker_path();
...
fn copy_generated_code_to_worker_dir() -> Result<()> {
    ...
    let wasm_dest = worker_path(format!("{OUT_NAME}_bg.wasm"));
...
fn bundle(esbuild_path: &Path) -> Result<()> {
    ...
    let path = base_worker_path().canonicalize()?;
...
fn base_worker_path() -> PathBuf {
    let path = env::args().nth(1).expect("no output path given");
    PathBuf::from(path).join(OUT_DIR).join(WORKER_SUBDIR)
}

pub fn worker_path(name: impl AsRef<str>) -> PathBuf {
    base_worker_path().join(name.as_ref())
}

pub fn output_path(name: impl AsRef<str>) -> PathBuf {
    let path = env::args().nth(1).expect("no output path given");
    PathBuf::from(path).join(OUT_DIR).join(name.as_ref())
}
